...
function* getDataSaga({
  payload: {
    ...
    : Generator<CallEffect<any> | PutEffect<TasksDataActionTypes>, void, ArrayLike<unknown>> {
  try {
    const data = yield call(rsf.database.read, `${userId}/${year}/${month}/${day}`);
    if (data) {
      yield put(getDataSuccess(Object.entries(data)
        .map((key: string | any) => ({ ...key[1], id: key[0] }))));
    } else {
...
function* patchDataSaga({
  payload: {
    ...
    : Generator<CallEffect<any> | PutEffect<TasksDataActionTypes>, void, unknown> {
  try {
